...
<head>
  ...
  <link rel="shortcut icon" href="img/imagens para uso do favicon/monitor.png" type="image/x-icon" />
...
<body>
    ...
    <nav>
      ...
      <nav class="sc-hAZoDl cqDFSz">
        <div><a href="inicio.php"><img
              src="img/icons a ser usadas na navbar/explorar.png" alt="Shop">Home</a></div>
        </div>
      </nav>
    </nav>
  </div>
  ...
  $sql = "SELECT * FROM produto WHERE categoria = 'Monitor'";
  ...
  if ($result) {
    if ($result->num_rows > 0) {
      ...
      echo "<center>Nenhum produto encontrado na categoria 'Monitor</center>s'.";
    }
  }
  ?>
  <?php
...
if(empty($produto_buscado)) {
  ...
  $sql = "SELECT * FROM produto where primeiro_nome like '%$produto_buscado%' AND categoria = 'Monitor' order by rand()";
...
?>
  <section class="sc-cxabCf cEZwIm3">
